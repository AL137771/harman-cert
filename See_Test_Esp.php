...
if (isset($_POST['Apply_Test'])) {
  ...
  $query = 'SELECT trainers.nameTrainer, area.nombreArea, operacion.nOperacion, empleados.nameEmpleado, 
                    empleados.idEmpleado, sp_recert.calification, esptesttaken.date
            FROM sp_recert
            ...
            INNER JOIN empleados ON empleados.idEmpleado = sp_recert.idEmpleado
            INNER JOIN esptesttaken ON sp_recert.idCertification = esptesttaken.idCertification
            WHERE sp_recert.idCertification ='.$idCertification;
  $j = $db->query($query);
    $row2 = $j -> fetch_assoc();
...
INNER JOIN sp_recert ON sp_recert.idOperacion = test.idOperacion
WHERE sp_recert.idCertification  ='.$idCertification;
          $i = $db->query($query);
  ...
  ?>
    ...
    <div id="page-content-wrapper">
      ...
      <nav class="navbar navbar-expand-lg navbar-light bg-light border-bottom">
        <button class="btn btn-info" id="menu-toggle">EXAMEN</button>
...
if ($_SESSION['typeUser'] == 1) {
 ...
 <button class="btn btn-danger" formaction="Trainer_SP_Recert.php" type="submit">
 <i class="fas fa-undo-alt"></i></button>
 </form>';
}
elseif ($_SESSION['typeUser'] == 2) {
  echo  '<form action="" method="POST">
  <button class="btn btn-danger" formaction="Admin_SP_Page.php"  type="submit">
  <i class="fas fa-undo-alt"></i></button>
  </form>';
}
...
while ($row = $i->fetch_assoc()) {
  ...
  echo '<input type="hidden" name="question_'.$cont.'" value="'.$row['idQuestion'].'">'.$row['question'];
                $query = 'SELECT espanswerstest.idQuestion, espanswerstest.idAnswer, answer.answer, answer.value
                from espanswerstest
                  ...
                  WHERE espanswerstest.idQuestion="'.$row['idQuestion'].'"'.'AND esptesttaken.idCertification ="'.$idCertification.'"';
                $j = $db->query($query);
                echo '<br>';
                echo '<br>';
