...
if (isset($_POST['Trace_Created_Recertification'])) {

    $idCertification = $_POST['idCertification'];

   $query = "SELECT * FROM recertsq WHERE idCertification = ". $idCertification;
...
<?php 
  ...
  <div id="page-content-wrapper">
    ...
    <form action=<?php if ($_SESSION['typeUser'] == 1) {
      echo '"Trainer_RecertPage.php"';
  }
  elseif ($_SESSION['typeUser'] == 2) {
      echo '"Admin_RecertPage.php"';
  } ?> method="POST">
                  ...
                  include "Table_Template.php";                 
                      while($row = $consulta -> fetch_assoc()){
                        ...
                        echo '<input type="hidden" name="idCertification" value="'.$row['idCertification'].'">';
...
<?php if ($_SESSION['typeUser'] == 1) {
  ...
  <button class="col-sm-4 btn btn-primary" type="submit" value="Guardar datos" name="Update_Trace_Recertification"><i class="fas fa-sign-in-alt"></i></button>
   </div> ';
   echo '<br><br>';
echo  '<div class="row">';
echo '<div class="col-sm-4"></div>';
echo ' <button class="col-sm-4 btn btn-danger" formaction="Trainer_RecertPage.php"  type="submit" value="Guardar datos" ><i class="fas fa-undo-alt"></i></i></button>
             </div>'; 
}
elseif ($_SESSION['typeUser'] == 2) {
echo  '<div class="row">';
echo '<div class="col-sm-3"></div>';
echo '<a href="Admin_RecertPage.php"  class="col-sm-3 btn btn-secondary btn-sm" role="button" aria-disabled="true">Volver</a>
             </div>'; 
} ?>
