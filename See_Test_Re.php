...
$query = 'SELECT trainers.nameTrainer, area.nombreArea, operacion.nOperacion, empleados.nameEmpleado, 
                    empleados.idEmpleado, recertsq.calification, retesttaken.date
            FROM recertsq
            ...
            INNER JOIN empleados ON empleados.idEmpleado = recertsq.idEmpleado
            INNER JOIN retesttaken ON recertsq.idCertification = retesttaken.idCertification
...
INNER JOIN recertsq ON recertsq.idOperacion = test.idOperacion
WHERE recertsq.idCertification  ='.$idCertification;
          $i = $db->query($query);
...
while ($row = $i->fetch_assoc()) {
  ...
  echo '<input type="hidden" name="question_'.$cont.'" value="'.$row['idQuestion'].'">'.$row['question'];
                $query = 'SELECT reanswerstest.idQuestion, reanswerstest.idAnswer, answer.answer, answer.value
                from reanswerstest
                  ...
                  WHERE reanswerstest.idQuestion="'.$row['idQuestion'].'"'.'AND retesttaken.idCertification ="'.$idCertification.'"';
                $j = $db->query($query);
                echo '<br>';
                echo '<br>';
  ...
  echo '<br>';
    ...
    <div class='row'>
                ...
                <?php
                  if ($_SESSION['typeUser'] == 1) {
                   echo '<button class="col-sm-3 btn btn-primary" formaction="Trainer_Recert_Complete.php"  type="submit"><i class="fas fa-undo-alt"></i></button>';
                  } else {
                    echo '<button class="col-sm-3 btn btn-primary" formaction="Admin_Recert_Complete.php"  type="submit"><i class="fas fa-undo-alt"></i></button>';
                  }
